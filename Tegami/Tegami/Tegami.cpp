...
void main() {
	...
	bg->Color(Time("00:00:000").ms, Time("10:00:000").ms, Color(150, 150, 150), Color(150, 150, 150));
	bg->ScaleVector(Time("00:00:000").ms, Time("10:00:000").ms, Vector2::ScreenSize, Vector2::ScreenSize);*/

	int effects = layer1 | layer2 | layer3 | layer4;
	...
	if (effects & 1) {
		...
		Storyboard::Instance()->Write("1.osb");
	}
	if (effects & 2) {
		Storyboard::Instance()->Write("2.osb");
	}
	if (effects & 4) {
		...
		Storyboard::Instance()->Write("3.osb");
	}
	if (effects & 8) {
		Storyboard::Instance()->Write("4.osb");
	}

	std::vector<std::string> files = { "1.osb" ,"2.osb" ,"3.osb" ,"4.osb" };
	...
	for (fileNo = 0; fileNo < noOfLayers; fileNo++) {
		std::ifstream partFile;
		partFile.open(files[fileNo]);
		std::string line;
