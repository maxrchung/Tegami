...
public:
	...
	DotGenerator(Utility *utility, int numberOfDots, float dotScale, float scaleOffset, float dotVelocity, float speedOffset, Color colorBase, int colorOffset, float fadeBase, float fadeOffset, float startTime, float endTime, const std::string& imagePath, float extraStartFade = 0.0f, bool expand = false, float expandAmount = 1.1f);
